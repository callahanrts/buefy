export const afterEachGlobal = (app) => (to, from) => {
    ...
    updates.forEach((item) => {
        ...
        const $el = document.querySelector(item.tag)
        if ($el) $el.setAttribute(item.attr || 'content', item.value)
    })

    app.config.globalProperties.$eventHub.emit('navigate', to.meta)
}
